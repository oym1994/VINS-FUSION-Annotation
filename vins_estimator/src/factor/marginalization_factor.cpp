...
{
    residuals.resize(cost_function->num_residuals()); //代价函数的维度即为残差向量的维度,等于其parameter_block_sizes之和?

    std::vector<int> block_sizes = cost_function->parameter_block_sizes(); //parameter_block_sizes为每个优化变量块的变量大小，以IMU残差为例，为[7,9,7,9])
    raw_jacobians = new double *[block_sizes.size()];
    jacobians.resize(block_sizes.size());

    for (int i = 0; i < static_cast<int>(block_sizes.size()); i++)
    {
        jacobians[i].resize(cost_function->num_residuals(), block_sizes[i]); //分块的雅克比,其行数都是残差的维度,列数为涉及到的状态向量数量
...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info)
{
    ...
    std::vector<double *> &parameter_blocks = residual_block_info->parameter_blocks; //状态窗口内的所以状态量
...
std::vector<double *> MarginalizationInfo::getParameterBlocks(std::unordered_map<long, double *> &addr_shift)
{
    ...
    {
        if (it.second >= m)  //parameter_block_idx中m以前的都是此次要被marginalize掉的
        {
